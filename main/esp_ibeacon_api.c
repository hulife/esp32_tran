...
esp_ble_ibeacon_head_t ibeacon_common_head = {
    .flags = {0x02, 0x01, 0x02}, //标准iBeacon flag 是 0x02, 0x01, 0x06
    .length = 0x1A,
    .type = 0xFF,
    .company_id = 0x004C,
    .beacon_type = 0x1502
...
esp_ble_ibeacon_head_t ibeacon_common_head_1 = {
    .flags = {0x02, 0x01, 0x1A}, //iphone 发送 ble 头部
    .length = 0x1A,
    .type = 0xFF,
    .company_id = 0x004C,
    .beacon_type = 0x1502
...
bool esp_ble_is_ibeacon_packet (uint8_t *adv_data, uint8_t adv_data_len){
    ...
    if ((adv_data != NULL) && (adv_data_len == 0x1E)){
        if (!memcmp(adv_data, (uint8_t*)&ibeacon_common_head, sizeof(ibeacon_common_head))  || !memcmp(adv_data, (uint8_t*)&ibeacon_common_head_1, sizeof(ibeacon_common_head_1)) ){
            result = true;
        }
    }
